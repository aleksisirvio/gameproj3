...
@onready var dance_game = preload("res://DanceGame.tscn")

const title : String = "Dance Pad"
...
func interact(interacter):
	...
	if interacter.tool == "":
		if task_manager.has_task(6): # Task.dance
			text_inst.set_text("")
			player.init_pause()
			get_parent().toggle_black()
			get_parent().add_child(dance_game.instantiate())
			ret = true
		else:
